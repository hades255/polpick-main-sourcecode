$(function () {
  ...
  var dtUserTable = $('.bot-list-table'),
    select = $('.select2'),
    statusObj = {
  ...
  if (dtUserTable.length) {
    dtUserTable.DataTable({
      ...
      serverSide: true,
      ajax: {
        url: `${window.location.protocol}//${window.location.host}/bots/getall`,
        method: 'post',
          ...
          render: function (data, type, full, meta) {
            ...
            if ($image) {
              ...
              var $state = states[stateNum],
                $name = full['full_name'] ? full['full_name'] : full['pool_id'],
            ...
            var $row_output =
              ...
              '<a href="' + window.location.protocol + '//' + window.location.host + '/bots/view/' + full["_id"] + '" class="user_name text-truncate text-body"><span class="fw-bolder">' +
              $name +
              '</span></a>' +
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            let actionBtn = (
              ...
              '<a href="' + window.location.protocol + '//' + window.location.host + '/bots/view/' + full["_id"] + '" class="dropdown-item d-none">' +
  ...
  $(document).on('click', '.userStatusUpdate', function () {
    ...
    swal.fire({
      ...
      if (result.value) {
        window.location.href = `${window.location.protocol}//${window.location.host}/bots/status-change/${elemID}?status=${result.value}`;
      }
    });
  });
  ...
  $(document).on('click', '.deleteUser', function () {
    ...
    swal.fire({
      ...
      if (result.value) {
        window.location.href = `${location.protocol}//${window.location.host}/bots/delete/${elemID}`;
      }
    });
  });
...
$(document).on('click', '.bulkDeleteBtn', function () {
  ...
  swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        url: `${location.protocol}//${window.location.host}/bots/bulk/delete`,
        type: "POST",
