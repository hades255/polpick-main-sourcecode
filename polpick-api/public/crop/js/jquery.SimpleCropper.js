...
(function ($) {
    $.fn.simpleCropper = function (onComplete) {
        ...
        var jcrop_api;
        var bottom_html = "<input type='file' id='article-image' name='article_image'/ hidden accept='image/png, image/gif, image/jpeg, image/bmp, image/tiff, image/gif'><canvas id='myCanvas' style='display:none;'></canvas><div id='modal'></div><div id='preview'><div class='buttons'><div class='cancel'></div><div class='ok'></div></div></div>";
        $('.appendBody').append(bottom_html);
        ...
        $(document).ready(function () {
            ...
            $('#article-image').change(function (click) {
        ...
        function imageUpload(dropbox) {
            var file = $("#article-image").get(0).files[0];
        ...
        function preview() {
            ...
            // let myObj = { "image_name": image_filename, "x": (x1 * kw), "y": (y1 * kh), "width": (sw * kw), "height": (sh * kh) };
            $('#x1').val((x1 * kw));
            $('#y1').val(y1 * kh);
            $('#width1').val(sw * kw);
            $('#height1').val(sh * kh);
